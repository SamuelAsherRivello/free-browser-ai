...
The system SHALL provide a top-level Stats view that displays the overall average completed-response duration and a table grouped by provider and model. Each table row MUST show the provider, model, successful response count, and average response duration. The view MUST provide distinct loading, empty, unavailable, and populated states and MUST NOT expose individual measurements or cumulative duration totals.

#### Scenario: Aggregates are available
- **WHEN** a visitor opens Stats and aggregate measurements exist
- **THEN** the view displays the overall average and the grouped provider/model table with response counts and average durations

#### Scenario: No aggregate measurements exist
- **WHEN** a visitor opens Stats before any valid measurement has been stored
- **THEN** the view presents an empty state without inventing an average or table rows

#### Scenario: Aggregate retrieval fails
- **WHEN** aggregate statistics cannot be retrieved
- **THEN** the Stats view presents an unavailable state while About, Settings, and Chat remain usable

### Requirement: Disclose the telemetry privacy boundary
The system SHALL clearly disclose that conversations and inference stay in the browser while a successful response sends only provider, model, and elapsed duration to shared analytics. The disclosure MUST state that prompts, responses, identities, device identifiers, and application timestamps are not stored in the analytics data store.
